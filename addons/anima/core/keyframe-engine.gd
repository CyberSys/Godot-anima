@tool
class_name AnimaKeyframeEngine
extends Node
...
#	}
#
static func parse_animation(
	animation_data: Dictionary,
	full_keyframes_data: Dictionary,
) -> Dictionary:
	...
	frame_keys.sort_custom(AnimaAnimationsUtils.new().sort_frame_index)
	...
	for frame_key in frame_keys:
		...
		var result := _calculate_frame_data(
			wait_time,
			animation_data,
			relative_properties,
			frame_key_value,
...
static func _is_valid_frame_key(key) -> bool:
	if typeof(key) == TYPE_INT or typeof(key) == TYPE_FLOAT:
		return true

	var frame_key_checker = RegEx.new()

	frame_key_checker.compile("\\d")
	return frame_key_checker.search(key) != null

static func _calculate_frame_data(
	wait_time: float,
	animation_data: Dictionary,
	relative_properties: Array,
	current_frame_key: float,
	frame_data: Dictionary,
	previous_key_value: Dictionary,
	initial_values: Array,
	animations_data: Array
) -> Dictionary:
